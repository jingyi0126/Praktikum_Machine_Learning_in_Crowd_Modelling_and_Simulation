...
def train_epoch(
    ...
    for data, _ in dataloader:
        data = data.view(-1, int(np.shape(data)[-1] * np.shape(data)[-2])).to(
            device
        )
        # print(data)
        ...
        #  Compute ELBO loss
        loss = elbo_loss(data, rec_data, mu, logvar)
        #  Compute gradients
        loss.backward()
...
def evaluate(model: object, dataloader: object, device) -> np.float64:
    ...
    for data, _ in dataloader:
        ...
        loss = elbo_loss(data, x_hat, mu, logvar)
        total_loss += loss.item()
    average_loss = total_loss / len(dataloader.dataset)
    return np.float64(average_loss)
...
def instantiate_vae(d_in, d_latent, d_hidden_layer, device):
